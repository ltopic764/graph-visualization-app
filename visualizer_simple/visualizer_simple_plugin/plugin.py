...
class SimpleVisualizer(VisualizerPlugin):
    ...
    def render(self, graph: Graph, **options) -> str:
        ...
        max_nodes_in_column = max(len(node_ids) for node_ids in columns.values()) if columns else 0

        width = max(WIDTH, (max_lvl + 2) * 200)
        height = max(HEIGHT, (max_nodes_in_column + 1) * 80)
